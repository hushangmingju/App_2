...
<div>
    <pre>    计算器  来源显示：姓名+电话+来源+计算+时间 <a href="./expoyuyue_list.html?type=calc">显示列表</a>
    预约页面  来源显示：姓名+电话+来源+时间 <a href="./expoyuyue_list.html?type=yuyue">显示列表</a>
    </pre>
</div>
    <div class="row">
                ...
                <div class="box-content">
                    <table class="table table-striped table-bordered responsive">
                        ...
                        <tbody>
							...
							$listArr = $db->QueryData("SELECT COUNT(*) AS `count` FROM `tb_expo_yuyue` WHERE $where","assoc");
							$allCount = isset($listArr["count"]) ? intval($listArr["count"]) : 0;
							$thisPageCount = 20;
							$page_len=10;
							include_once(dirname(__FILE__)."/page.cls.php");
							$listArr = $db->QueryData("SELECT * FROM `tb_expo_yuyue` WHERE $where	 ORDER BY `id` DESC LIMIT ". (($pageid-1) * $thisPageCount) ." , $thisPageCount","all");
							$count = count($listArr);
							$count = count($listArr);
							for($i=0;$i<$count;$i++){
                        ...
                        <tr>
                            ...
                            <td class="center"><a class="btn btn-danger" href="expo_yuyue_del.html?id=<?=$listArr[$i]["id"]?>"><i class="glyphicon glyphicon-trash icon-white"></i>删除</a></td>
