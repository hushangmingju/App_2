      ...
      <div class="box-content">
        <table class="table table-striped table-bordered responsive">
		  ...
		  if ($getType=='calc' || $getType=='yuyue') {
		      $listArr->setWhere("`t2`.`status` = 1 AND `t2`.`folder` NOT IN ('root', 'mobile') AND `t1`.`status`='ok' AND `t1`.`group_table` = 4 AND `t1`.`type`='" . $getType . "'");
			  $defUrl= "type=".$getType;
		  } else {
              $listArr->setWhere("`t2`.`status` = 1 AND `t2`.`folder` NOT IN ('root', 'mobile') AND `t1`.`status`='ok' AND `t1`.`group_table` = 4");
			  $defUrl= "";
		  }
