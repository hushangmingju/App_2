...
// SELECT COUNT(*) AS `sumHitPoint`, COUNT(DISTINCT `guestID`) AS `sumGID`, COUNT(DISTINCT `ip`) AS `sumIP` FROM `tb_vcs_guest` WHERE `isSpider` = 0 AND `isRedirect` = 0  AND `timestamp` > DATE(NOW())
...
//$sumHitPoint =  
$reserveToday = new Query("COUNT(*) AS `sumReserve`, COUNT(DISTINCT `gid`) AS `sumGID`", "`yuyue`", "", "`status` = 'ok' AND `time` >= UNIX_TIMESTAMP(DATE(NOW()))");
$reserveToday = DAS::quickQuery($reserveToday);
  ...
  <div class="box col-md-12">
    <div class="box-inner">
      ...
      <div class="box-content">
        <span>日预约数：</span><span><?php echo (DAS::hasData($reserveToday) ? $reserveToday['data'][0]['sumReserve'] : '无数据')?></span><br/>
        <span>日预约人数：</span><span><?php echo (DAS::hasData($reserveToday) ? $reserveToday['data'][0]['sumGID'] : '无数据')?></span><br/>
      </div>
    </div>
  </div>
</div>
                ...
                <div class="box-content">
                    <table class="table table-striped table-bordered responsive">
                        ...
                        <tr>
                            ...
                            <td class="center"><?=rawurldecode($listArr[$i]["url"])?></td>
