...
$contents = new Query("*", "`tb_wcp_contents`", "", "`pageID` = 50 AND `status` = 1");
...
$banner = new Query("*", "`tb_wcp_images`", "", "`status` = 1 AND `component` = 'm_about_banner'");
$banner = DAS::quickQuery($banner);
$banner = DAS::hasData($banner) ? $banner['data'][0] : false;

$pictures = new Query("*", "`tb_wcp_images`", "", "`status` = 1 AND `pageID` = 50 AND `component` = 'm_about_pic'", "`ordnung`");
            ...
            <div class="box col-md-12">
              <div class="box-inner">
                <div class="box-header well">
                  <h2><i class="glyphicon glyphicon-info-sign"></i> <a href="http://www.mingjugroup.com/mobile/about.html" target="_blank">移动端关于我们 mobile/about.html</a></h2>
                </div>
                <div class="box-content row">
                  <div class="box col-md-12">
                    ...
                    <div class="box-inner">
                      ...
                      <div class="box-content" style="text-align:left; padding:0px 0px 24px 0px; overflow:hidden; display:none;">
                        <form>
                          ...
                          <input type="hidden" name="pageFile" value="mobile/about.php"/>
                    ...
                    <div class="box-inner" style="margin-top:24px;">
                      <div class="box-header well" data-original-title="" onClick="$(this).next('div').slideToggle('slow');" style="cursor:pointer;">
                        <h2 style="display:inline-block;">图片管理</h2><span style="font-size:12px; margin-left:24px; font-weight:100; color:#888;">此处可以添加、删除、编辑首页的Banner图片，还可以调整图片的显示顺序。</span>
                      </div>
...
function addBanner(){
  ...
  // 设置component
  formJQuery.children("input").eq(2).val("m_about_banner");
  // 设置folder
  formJQuery.children("input").eq(3).val("pics");
  // 设置descript
  formJQuery.children("input").eq(4).val("移动端关于我们页面Banner图片");
  // 设置pageFile
  formJQuery.children("input").eq(5).val("mobile/about.php");
  // 设置对话框标题
  formJQuery.find("div.DialogTitle").text("添加移动端关于我们页面Banner图片:");
  // 清空临时文件路径
...
function setBanner(){
  ...
  formJQuery.children("input").eq(5).val("mobile/about.php");
  // 设置对话框标题
  formJQuery.find("div.DialogTitle").text("设置移动端关于我们页面Banner图片:");
  // 清空临时文件路径
  formJQuery.find("#TEMP_UPLOAD_IMG_PATH").val("");
  // 清空临时文件ID
  formJQuery.find("#TEMP_UPLOAD_IMG_ID").val("");
...
function addPic(){
  ...
  formJQuery.children("input").eq(1).val("add_image");
  // 设置component
  formJQuery.children("input").eq(2).val("m_about_pic");
  // 设置folder
  formJQuery.children("input").eq(3).val("pics");
  // 设置descript
  formJQuery.children("input").eq(4).val("移动端关于我们页面的图片");
  // 设置pageFile
  formJQuery.children("input").eq(5).val("mobile/about.php");
  // 设置对话框标题
  formJQuery.find("div.DialogTitle").text("添加关于我们页面的图片:");
  // 清空临时文件路径
  formJQuery.find("#TEMP_UPLOAD_IMG_PATH").val("");
  // 清空临时文件ID
  formJQuery.find("#TEMP_UPLOAD_IMG_ID").val("");
...
function setPic(ordnung){
  ...
  formJQuery.children("input").eq(5).val("mobile/about.php");
  // 设置对话框标题
  formJQuery.find("div.DialogTitle").text("设置关于我们页面的图片:");
  // 清空临时文件路径
  formJQuery.find("#TEMP_UPLOAD_IMG_PATH").val("");
  // 清空临时文件ID
  formJQuery.find("#TEMP_UPLOAD_IMG_ID").val("");
...
function deletePic(id){
  if(!confirm("确定要删除关于我们页面的图片吗？")){
    return false;
  }
  $.ajax({
    url: "wcp_svr.html",
    type: "POST",
    data: {"action": "delete_image", "id": id, "component": "m_about_pic"},
    dataType: "json",
    success: function(data) { 
      alert(data.TEXT);
      reloadPage();
...
function movePrevPic(id){
  $.ajax({
    url: "wcp_svr.html",
    type: "POST",
    data: {"action": "moveprev_image", "id": id, "component": "m_about_pic"},
    dataType: "json",
    success: function(data) { 
      alert(data.TEXT);
      reloadPage();
...
function moveNextPic(id){
  $.ajax({
    url: "wcp_svr.html",
    type: "POST",
    data: {"action": "movenext_image", "id": id, "component": "m_about_pic"},
    dataType: "json",
    success: function(data) { 
      alert(data.TEXT);
      reloadPage();
