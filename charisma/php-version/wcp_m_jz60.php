...
$contents = new Query("*", "`tb_wcp_contents`", "", "`pageID` = 54 AND `status` = 1");
...
$banner = new Query("*", "`tb_wcp_images`", "", "`status` = 1 AND `component` = 'm_jz60_banner'");
$banner = DAS::quickQuery($banner);
$banner = DAS::hasData($banner) ? $banner['data'][0] : false;

$pictures = new Query("*", "`tb_wcp_images`", "", "`status` = 1 AND `pageID` = 54 AND `component` = 'm_jz60_pic'", "`ordnung`");
$pictures = DAS::quickQuery($pictures);
$pictures = DAS::hasData($pictures) ? $pictures['data'] : false;

$composes = new Query("*", "`tb_wcp_composes`", "", "`pageID` = 54 AND `status` = 1", "`id`");
            ...
            <div class="box col-md-12">
              <div class="box-inner">
                <div class="box-header well">
                  <h2><i class="glyphicon glyphicon-info-sign"></i> <a href="http://www.mingjugroup.com/mobile/jz60.html" target="_blank">移动端家装6.0 mobile/jz60.html</a></h2>
                </div>
                  ...
                  <div class="box col-md-12">
                    ...
                    <div class="box-inner">
                      ...
                      <div class="box-content" style="text-align:left; padding:0px 0px 24px 0px; overflow:hidden; display:none;">
                        <form>
                          ...
                          <input type="hidden" name="pageFile" value="mobile/jz60.php"/>
                    ...
                    <div class="box-inner" style="margin-top:24px;">
                      ...
                      <div class="box-content" style="text-align:left; padding:0px 0px 24px 0px; overflow:hidden; background-color:#FFFFFF; display:none;"> 
                        ...
                        <div style="display:block; width:100%; margin-top:24px; text-align:center;">
                          <button type="button" onClick="setBanner();" style="background-color:#35a6e7; border-radius:10px; width:200px; padding:5px; border:none; font-size:22px; color:#FFF;">编辑Banner图片</button>  
                        </div>
                      </div>
                    </div>
                    <!-- 家装6.0图片编辑栏-->
                    <div class="box-inner" style="margin-top:24px;">
                      <div class="box-header well" data-original-title="" onClick="$(this).next('div').slideToggle('slow');" style="cursor:pointer;">
                        <h2 style="display:inline-block;">图片管理</h2><span style="font-size:12px; margin-left:24px; font-weight:100; color:#888;">此处可以添加、删除、编辑家装6.0页面的图片，还可以调整图片的显示顺序。</span>
                      </div>
                    ...
                    <div class="box-inner" style="margin-top:24px;">
                      <div class="box-header well" data-original-title="" onClick="$(this).next('div').slideToggle('slow');" style="cursor:pointer;">
                        <h2 style="display:inline-block;">插件管理</h2><span style="font-size:12px; margin-left:24px; font-weight:100; color:#888;">此处可以添加、删除、编辑家装6.0页面的插件。</span>
                      </div>
                      <div class="box-content" style="text-align:left; padding:24px 64px 24px 64px; display:none;">
                        <?php
                        ...
                        <div style="display:inline-block; width:24%; padding:5px; text-align:left; min-width:330px; margin-top:10px;">
                          ...
                          <div style="display:inline-block; width:320px; height:64px; text-align:left; background-color:#FFFFFF; padding:5px 5px 5px 5px; border:1px solid #CCCCCC; border-top:none;"> 
                            ...
                            <a class="icon_button3" onClick="deletePlugIn(<?php echo $compose['id']?>)" style="font-size:14px; text-align:center; display:inline-block; width:90px; height:24px; float:right; margin-top:14px; padding:3px; margin-right:-5px;">
...
function addBanner(){
  ...
  // 设置component
  formJQuery.children("input").eq(2).val("m_jz60_banner");
  // 设置folder
  formJQuery.children("input").eq(3).val("pics");
  // 设置descript
  formJQuery.children("input").eq(4).val("移动端家装6.0页面Banner图片");
  // 设置pageFile
  formJQuery.children("input").eq(5).val("mobile/jz60.php");
  // 设置对话框标题
  formJQuery.find("div.DialogTitle").text("添加移动端家装6.0页面Banner图片:");
  // 清空临时文件路径
...
function setBanner(){
  ...
  formJQuery.children("input").eq(5).val("mobile/jz60.php");
  // 设置对话框标题
  formJQuery.find("div.DialogTitle").text("设置移动端家装6.0页面Banner图片:");
  // 清空临时文件路径
  formJQuery.find("#TEMP_UPLOAD_IMG_PATH").val("");
  // 清空临时文件ID
  formJQuery.find("#TEMP_UPLOAD_IMG_ID").val("");
...
function addPic(){
  ...
  formJQuery.children("input").eq(1).val("add_image");
  // 设置component
  formJQuery.children("input").eq(2).val("m_jz60_pic");
  // 设置folder
  formJQuery.children("input").eq(3).val("pics");
  // 设置descript
  formJQuery.children("input").eq(4).val("移动端家装6.0图片");
  // 设置pageFile
  formJQuery.children("input").eq(5).val("mobile/jz60.php");
  // 设置对话框标题
  formJQuery.find("div.DialogTitle").text("添加家装6.0图片:");
  // 清空临时文件路径
  formJQuery.find("#TEMP_UPLOAD_IMG_PATH").val("");
  // 清空临时文件ID
  formJQuery.find("#TEMP_UPLOAD_IMG_ID").val("");
  ...
  formJQuery.find("p#TEMP_UPLOAD_IMG_INFO").text("");
  // 设置图片上传要求
  formJQuery.find("p#TEMP_UPLOAD_IMG_REQUIRE").text("图片尺寸为宽大于640px，文件小于512KB的.jpg或.png格式图片。");   
  // 清空预览图片
  formJQuery.find("div#TEMP_UPLOAD_IMG_PREVIEW_DIV").html(""); 
...
function setPic(ordnung){
  ...
  formJQuery.children("input").eq(5).val("mobile/jz60.php");
  // 设置对话框标题
  formJQuery.find("div.DialogTitle").text("设置家装6.0图片:");
  // 清空临时文件路径
  formJQuery.find("#TEMP_UPLOAD_IMG_PATH").val("");
  // 清空临时文件ID
  formJQuery.find("#TEMP_UPLOAD_IMG_ID").val("");
...
function deletePic(id){
  if(!confirm("确定要删除此家装6.0图片吗？")){
    return false;
  }
  $.ajax({
    url: "wcp_svr.html",
    type: "POST",
    data: {"action": "delete_image", "id": id, "component": "m_jz60_pic"},
    dataType: "json",
    success: function(data) { 
      alert(data.TEXT);
      reloadPage();
...
function movePrevPic(id){
  $.ajax({
    url: "wcp_svr.html",
    type: "POST",
    data: {"action": "moveprev_image", "id": id, "component": "m_jz60_pic"},
    dataType: "json",
    success: function(data) { 
      alert(data.TEXT);
      reloadPage();
...
function moveNextPic(id){
  $.ajax({
    url: "wcp_svr.html",
    type: "POST",
    data: {"action": "movenext_image", "id": id, "component": "m_jz60_pic"},
    dataType: "json",
    success: function(data) { 
      alert(data.TEXT);
      reloadPage();
...
function setPlugIn(id){
  ...
  $("#PLUGIN_SUBMIT_BUTTON").click(function(e) {
    ...
    $.ajax({
      url: "wcp_svr.html",
      type: "POST",
      data: {"id":id, "action":"set_plugin", "name":name, "pageFile":"mobile/jz60.php", "plugInIndex":$(this).parent().parent().find("input").eq(1).val(), "json":values},
      dataType: "json",
      success: function(data) { 
        alert(data.TEXT);
        reloadPage();
...
function deletePlugIn(id){
  ...
  $.ajax({
    url: "wcp_svr.html",
    type: "POST",
    data: {"id":id, "action":"delete_plugin", "id":id},
    dataType: "json",
    success: function(data) { 
      alert(data.TEXT);
      history.go(0);
