...
// 检验GET提供的页面ID是否正确
if (!isset($_GET['id']) || !$_GET['id'] || !is_int(intval($_GET['id'])) || intval($_GET['id']) < 1 || !DAS::isExistedInDB("`tb_vcs_pages`", "`id` = " . intval($_GET['id']))) {
    die("<script type='text/javascript'>window.location.href='wcp_root.html'</script>");
}

// 页面参数
$pageInfos = new Query("", "`tb_vcs_pages`", "", "`id` = " . intval($_GET['id']));
$pageInfos = DAS::quickQuery($pageInfos);
$pageInfos = DAS::hasData($pageInfos) ? $pageInfos['data'][0] : false;

$pageFile = $pageInfos['pageFile'];
$imagesFolder = $pageInfos['imagesFolder'];
                      ...
                      <div class="box-content" style="text-align:left; padding:0px 0px 24px 0px; overflow:hidden; display:none;">
                        <form>
                          ...
                          <div style="width:100%; box-shadow:#888888 0px 3px 10px; padding-top:24px;"> 
                            ...
                            <?php
                            if (strpos($pageFile, 'mobile/anli') !== false || strpos($pageFile, 'mobile/zixun') !== false) {
                            ?>
                            <div style="display:block; width:90%; margin-top:20px; padding:0px 64px 0px 64px;">
                              <div style="display:block; text-align:left;">
                                <span style="color:#35a6e7; font-size:16px; font-weight:bold;">首页显示标题:</span>
                                <span>首页相关模块的主标题，建议字数小于8</span>
                              </div>
                              <input type="text" name="maintitle" value="<?php echo isset($values['maintitle']) ? $values['maintitle'] : '';?>" style="width:100%; font-size:14px;  padding:6px; border-radius:5px; border:1px solid #aaa; margin-top:8px;"/>
                            </div>
                            <div style="display:block; width:90%; margin-top:20px; padding:0px 64px 0px 64px;">
                              <div style="display:block; text-align:left;">
                                <span style="color:#35a6e7; font-size:16px; font-weight:bold;">首页显示副标题:</span>
                                <span>首页相关模块的副标题，建议字数小于16</span>
                              </div>
                              <input type="text" name="subtitle" value="<?php echo isset($values['subtitle']) ? $values['subtitle'] : '';?>" style="width:100%; font-size:14px;  padding:6px; border-radius:5px; border:1px solid #aaa; margin-top:8px;"/>
                            </div>
                            <?php
                            }
                            ?>
                            ...
                            <div style="display:inline-block; width:90%; margin-top:20px; padding:0px 64px 0px 64px;">
                              <div style="display:block; text-align:left;">                              
                                <span style="color:#35a6e7; font-size:16px; font-weight:bold;">页面中显示Banner图:</span>
                                <select name="showBanner" style="width:240px; font-size:14px; padding:6px; border-radius:5px; border:1px solid #aaa; margin-top:8px; margin-left:0px;">
                                  <option value="1" <?php echo isset($values['showBanner']) && $values['showBanner'] == 1 ? 'selected="selected"' : '';?>>显示</option>
                                  <option value="0" <?php echo isset($values['showBanner']) && $values['showBanner'] == 0 ? 'selected="selected"' : '';?>>隐藏</option>
                                </select>
                                <span style="margin-left:20px;">如果选择隐藏,页面中Banner图将不会显示。</span> 
                              </div>                            
                            </div>                      
...
<!--//
var values = <?php echo ($values ? json_encode($values) : 'false');?>;
...
function setBanner(id){
  ...
  // 设置图片上传要求
  formJQuery.find("p#TEMP_UPLOAD_IMG_REQUIRE").text("图片尺寸为640x480px或同比例扩大，文件小于512KB的.jpg、.gif或.png格式图片。");      
  // 清空预览图片
  ...
  if(id && id == <?php echo $banner ? $banner['id'] : -1;?>){
    ...
    // 设置head信息
    setField(contentsJQuery.find("#IMG_DIALOG_HEAD_DIV"), true, "<?php echo $banner ? $banner['head'] : '';?>");
    // 隐藏content信息
    setField(contentsJQuery.find("#IMG_DIALOG_CONTENT_DIV"), true, "<?php echo $banner ? $banner['content'] : '';?>");
    // 隐藏css信息
    setField(contentsJQuery.find("#IMG_DIALOG_CSS_DIV"), false);
