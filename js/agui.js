...
DIALOG.prototype.show = function(evt, callback){
	if(this.isready){
		var bgObj = createShadowBackground(this.id, evt);
		if(bgObj){
...
DIALOG.prototype.hide = function(callback){
	if(this.isready && document.getElementById("AGUI_SHADOW_BACKGROUND_DIV" + this.id)){
		document.body.removeChild(document.getElementById("AGUI_SHADOW_BACKGROUND_DIV" + this.id));
		var dgObj = document.getElementById(this.id);
...
// HTTP document.DOMElement createShadowBackground(id, [(Event) evt])
function createShadowBackground(id, evt){
	if(document.getElementById("AGUI_SHADOW_BACKGROUND_DIV" + id)){
		return document.getElementById("AGUI_SHADOW_BACKGROUND_DIV" + id);
	}
	var csbBackground = document.createElement("div");
	csbBackground.id = "AGUI_SHADOW_BACKGROUND_DIV" + id;
    ...
    csbBackground.style.zIndex = document.getElementById(id) && document.getElementById(id).style.zIndex ? document.getElementById(id).style.zIndex - 5 : "110";
