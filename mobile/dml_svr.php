...
switch ($_POST['type']) {    
    ...
    case 'calc':
        if (strpos($region['prov'], '福建') !== false) {
            $priceList = new Query("priceList", "`tb_wcp_showrooms`", "", "id = 49"); 
        }
        else {
            $priceList = new Query("priceList", "`tb_wcp_showrooms`", "", "id = 1"); 
        }
...
$url = _POST("url");
$url = $url ? $url : rawurlencode($_SERVER['HTTP_REFERER']);

$reserveDML->setValue('url', $url);
if (strpos($url, 'yangban-v') !== false && strpos($url, 'id%3D') !== false) {
    $showroomID = intval(substr($url, (strpos($url, 'id%3D') + 5)));
