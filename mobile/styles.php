...
if (strpos($region['prov'], '福建') !== false) {
    $tagGroupZZ = new Query("`t1`.*, `t2`.`number`, `t2`.`shop`, `t2`.`name`, `t2`.`kujiale`, `t2`.`timestamp`, `t3`.`name` AS shopName, `t4`.`folderName`, `t4`.`fileName`", "`tb_wcp_tags` AS `t1`", "LEFT JOIN `tb_wcp_showrooms` AS `t2` ON `t1`.`itemID` = `t2`.`id` LEFT JOIN `tb_wcp_shops` AS `t3` ON `t3`.`id` = `t2`.`shop` LEFT JOIN `tb_wcp_images` AS `t4` ON `t4`.`showroomNum` = `t2`.`number` AND `t4`.`showroomShop` = `t2`.`shop` AND `t4`.`component` LIKE 'sr%cover'", "`t1`.`tagGroup` = '" . $values['taggroup'] . "' AND `t1`.`itemType` = 1 AND `t2`.`status` = 1 AND `t2`.`shop` = 7", "`t1`.`tagIndex`, `t1`.`itemIndex`");
    $tagGroupZZ = DAS::quickQuery($tagGroupZZ);
    $tagGroupZZ = $tagGroupZZ['data'];
    foreach ($tagGroupZZ as $item) {
...
if (strpos($region['prov'], '福建') !== false) {
    $showroomsZZ = new Query("`t1`.*, `t2`.`name` AS shopName", "`tb_wcp_showrooms` AS `t1`", "LEFT JOIN `tb_wcp_shops` AS `t2` ON `t1`.`shop` = `t2`.`id`", "`t1`.`status` = 1 AND `t1`.`shop` = 7", "`t1`.`toTop` DESC, `t1`.`ordnung`, `t1`.`id` DESC, `t1`.`number`");
    $showroomsZZ = DAS::quickQuery($showroomsZZ);
    $showroomsZZ = DAS::hasData($showroomsZZ) ? $showroomsZZ['data'] : false;
}

$showrooms = new Query("`t1`.*, `t2`.`name` AS shopName", "`tb_wcp_showrooms` AS `t1`", "LEFT JOIN `tb_wcp_shops` AS `t2` ON `t1`.`shop` = `t2`.`id`", "`t1`.`status` = 1 AND `t1`.`shop` != 7", "`t1`.`toTop` DESC, `t1`.`ordnung`, `t1`.`id` DESC, `t1`.`number`");
...
?>  
    ...
    if (!$styleTag) {
        if (strpos($region['prov'], '福建') !== false) {
            for ($i = 0; $i < count($showroomsZZ); $i++) {
                foreach ($covers as $cover) {
                    if ($cover['showroomNum'] == $showroomsZZ[$i]['number'] && $cover['showroomShop'] == $showroomsZZ[$i]['shop']) {
                        $showroomCover = $cover;
                    }
                }
                if ($sectSplit == 1) {
    ...
    <section style="text-align:center; margin-top:36px;">
      <div style="width:91%; display:inline-block; text-align:left; max-width:<?php echo $maxWidth;?>;">
        <a href="yangban-v.html?id=<?php echo $showroomsZZ[$i]['id'];?>"> 
          ...
          <?php
          if ($showroomsZZ[$i]['kujiale']) { 
          ?> 
            <img src="../images/icons/vr_logo3.png" style="position:absolute; width:8%; margin-top:3px; margin-left:31%;"/>
          <?php
          }
          ?>
    ...
    <section style="text-align:center; margin-top:36px;">
      <div style="width:91%; display:inline-block; text-align:left; max-width:<?php echo $maxWidth;?>;">
        <a href="yangban-v.html?id=<?php echo $showrooms[$i]['itemID'];?>"> 
          <div class="covers" style="width:47.5%; float:left; display:inline-block; box-shadow: 2px 10px 15px rgba(0, 0, 0, 0.5); background-image:url(<?php echo $showroomCover['folderName'] . '/' . $showroomCover['fileName'];?>); background-size:cover; text-align:center;">
          <?php
          if ($showrooms[$i]['kujiale']) { 
          ?> 
